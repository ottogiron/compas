...
/// Parse markdown body text and return styled ratatui lines, each prefixed with `\u{2502} `.
///
/// Used in the conversation view where body text is subordinate to `from \u{2192} to` headers.
pub(crate) fn markdown_to_lines(body: &str, border_color: Color) -> Vec<Line<'static>> {
    let prefix = Span::styled("\u{2502} ", Style::default().fg(border_color));
    markdown_to_lines_inner(body, Some(prefix))
}

/// Parse markdown body text and return styled ratatui lines without any prefix.
///
/// Used in the Input/Output tabs of execution detail where there is no parent header.
pub(crate) fn markdown_to_lines_standalone(body: &str) -> Vec<Line<'static>> {
    markdown_to_lines_inner(body, None)
}

/// Flush accumulated spans into a result line, optionally prepending a prefix span.
fn flush_line_inner(
    ...
    let mut line_spans = match prefix {
        Some(p) => vec![p.clone()],
        None => vec![],
    };
...
/// Push a blank line — prefix-only when bordered, empty when standalone.
fn push_blank_line(result: &mut Vec<Line<'static>>, prefix: &Option<Span<'static>>) {
    match prefix {
        Some(p) => result.push(Line::from(vec![p.clone()])),
        None => result.push(Line::from("")),
    }
}

/// Check whether a line is a trailing blank (prefix-only or empty).
fn is_trailing_blank(line: &Line<'_>, prefix: &Option<Span<'static>>) -> bool {
    match prefix {
        Some(_) => line.spans.len() == 1 && line.spans[0].content.as_ref() == "\u{2502} ",
        None => line.spans.is_empty() || line.spans.iter().all(|s| s.content.is_empty()),
    }
}

/// Shared markdown \u{2192} styled lines renderer. When `prefix` is `Some`, each line is
/// prepended with that span (conversation view). When `None`, lines stand alone.
fn markdown_to_lines_inner(body: &str, prefix: Option<Span<'static>>) -> Vec<Line<'static>> {
    ...
    for event in parser {
        match event {
            Event::Start(Tag::Heading { level, .. }) => {
                // Blank line above heading (unless first output line).
                if !result.is_empty() {
                    push_blank_line(&mut result, &prefix);
                }
                in_heading = true;
            ...
            Event::End(TagEnd::Heading(_)) => {
                flush_line_inner(&mut current_spans, &mut result, &prefix);
                style_stack.pop();
                in_heading = false;
            }
            ...
            Event::Code(text) => {
                current_spans.push(Span::styled(
                    text.to_string(),
                    Style::default().fg(TEXT_CODE),
                ));
            }
            Event::Text(text) => {
                if in_code_block {
                    // Code blocks: 4-space indent + card background for visual distinction.
                    let code_style = Style::default().fg(TEXT_NORMAL).bg(BG_CARD);
            ...
            Event::Rule => {
                ...
                let mut rule_spans = match &prefix {
                    Some(p) => vec![p.clone()],
                    None => vec![],
                };
                ...
                result.push(Line::from(rule_spans));
            }
    ...
    while result.last().is_some_and(|l| is_trailing_blank(l, &prefix)) {
        result.pop();
    }
...
fn push_message_lines(msg: &MessageRow, lines: &mut Vec<Line<'static>>) {
    ...
    // Header line: "from \u{2192} to  [intent]  timestamp"
    ...
    // Body — markdown-rendered with "\u{2502} " prefix per line
    let body = &msg.body;
    if body.is_empty() {
        lines.push(Line::from(vec![
            "\u{2502} ".fg(TEXT_DIM),
...
fn push_execution_started_line(agent: &str, started_at: i64, lines: &mut Vec<Line<'static>>) {
    ...
    lines.push(Line::from(vec![
        format!("  \u{25b6} Execution started ({})", agent).fg(TEXT_DIM),
        "   ".into(),
        ts.fg(TEXT_DIM),
    ]));
}
...
fn push_execution_completed_line(
    ...
    if success {
        lines.push(Line::from(vec![
            format!("  \u{2713} Completed{} ({})", dur, agent).fg(SUCCESS_DIM),
            "   ".into(),
            ts.fg(TEXT_DIM),
        ]));
    } else {
        lines.push(Line::from(vec![
            format!("  \u{2717} Failed{} ({})", dur, agent).fg(FAILURE),
            "   ".into(),
            ts.fg(TEXT_DIM),
        ]));
    }
...
mod tests {
    ...
    fn test_push_message_lines_operator_header() {
        ...
        // First line is header: "operator \u{2192} orch-dev-2  [dispatch]  <ts>"
    ...
    fn test_push_message_lines_body_with_prefix() {
        ...
        // Lines: header, "\u{2502} line one line two" (no borders — CONV-3 removed them)
    ...
    /// Helper: collect all span text from markdown_to_lines_standalone output.
    fn md_text_standalone(body: &str) -> Vec<String> {
        markdown_to_lines_standalone(body)
            .iter()
            .map(|line| {
                line.spans
                    .iter()
    ...
    fn test_markdown_inline_code() {
        ...
        assert_eq!(code_span.style.fg, Some(TEXT_CODE));
    ...
    fn test_markdown_code_block_has_visual_spacing() {
        ...
        assert_eq!(lines[code_idx + 1], "\u{2502} ");
    }
    ...
    fn test_markdown_code_block_has_indent() {
        ...
        // Code lines should have 4-space indent.
        assert!(
            code_line.contains("\u{2502}     fn main()"),
            "Expected 4-space indent in: {}",
            code_line
        );
    }
    ...
    fn test_markdown_thematic_break() {
        ...
        let has_rule = lines
            .iter()
            .any(|l| l.contains("\u{2500}\u{2500}\u{2500}\u{2500}"));
    ...
    fn test_markdown_all_lines_have_prefix() {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            assert_eq!(
                ...
                "\u{2502} ",
                "Line {} should start with \u{2502} prefix",
                i
            );
        }
    }
    ...
    #[test]
    fn test_standalone_no_prefix() {
        let lines = markdown_to_lines_standalone("hello world");
        assert_eq!(lines.len(), 1);
        // Standalone lines should NOT start with "\u{2502} ".
        let first_content = &lines[0].spans[0].content;
        assert_ne!(first_content.as_ref(), "\u{2502} ");
        let text: String = lines[0].spans.iter().map(|s| s.content.as_ref()).collect();
        assert!(text.contains("hello world"));
    }

    #[test]
    fn test_standalone_empty_body() {
        let lines = markdown_to_lines_standalone("");
        assert!(lines.is_empty());
    }

    #[test]
    fn test_standalone_heading_style() {
        let lines = markdown_to_lines_standalone("# Title");
        assert_eq!(lines.len(), 1);
        let text_span = &lines[0].spans[0];
        assert_eq!(text_span.style.fg, Some(ACCENT));
        assert!(text_span.style.add_modifier.contains(Modifier::BOLD));
    }

    #[test]
    fn test_standalone_heading_blank_line_above() {
        let lines = md_text_standalone("some text\n\n## Heading");
        // Should be: "some text", blank, blank (above heading), "Heading"
        assert!(lines.len() >= 3, "Expected at least 3 lines: {:?}", lines);
        let heading_idx = lines.iter().position(|l| l.contains("Heading")).unwrap();
        // Line before heading should be blank.
        assert!(
            lines[heading_idx - 1].is_empty(),
            "Expected blank line above heading: {:?}",
            lines
        );
    }

    #[test]
    fn test_standalone_code_block_indent() {
        let lines = md_text_standalone("```\nfn main() {}\n```");
        let code_line = lines.iter().find(|l| l.contains("fn main()")).unwrap();
        // 4-space indent, no prefix.
        assert!(
            code_line.starts_with("    fn main()"),
            "Expected 4-space indent in: {}",
            code_line
        );
    }
    ...
    #[test]
    fn test_standalone_code_block_text_normal_color() {
        let lines = markdown_to_lines_standalone("```\ncode\n```");
        let code_line = lines
            .iter()
        ...
        let code_span = code_line
            ...
            .unwrap();
        assert_eq!(code_span.style.fg, Some(TEXT_NORMAL));
    }

    #[test]
    fn test_standalone_inline_code_themed() {
        let lines = markdown_to_lines_standalone("`code`");
        let code_span = &lines[0].spans[0];
        assert_eq!(code_span.style.fg, Some(TEXT_CODE));
    }

    #[test]
    fn test_standalone_horizontal_rule_wide() {
        let lines = markdown_to_lines_standalone("above\n\n---\n\nbelow");
        let rule_line = lines
            .iter()
        ...
        let rule_span = rule_line
            .spans
            .iter()
            .find(|s| s.content.contains("\u{2500}"))
            .unwrap();
        // Dynamic-width: 200 chars.
        assert_eq!(rule_span.content.len(), 200 * "\u{2500}".len());
    }

    #[test]
    fn test_standalone_no_panic_on_malformed() {
        let _ = markdown_to_lines_standalone("```");
        let _ = markdown_to_lines_standalone("**unclosed bold");
        let _ = markdown_to_lines_standalone("# ");
        let _ = markdown_to_lines_standalone("---\n---\n---");
    }

    // ── Heading blank-line-above in bordered mode ────────────────────────

    #[test]
    fn test_bordered_heading_blank_line_above() {
        let lines = md_text("some text\n\n## Heading");
        let heading_idx = lines.iter().position(|l| l.contains("Heading")).unwrap();
        // Line before heading should be blank (prefix-only).
        assert_eq!(
            lines[heading_idx - 1],
            "\u{2502} ",
            "Expected blank prefix line above heading: {:?}",
            lines
        );
    }

    #[test]
    fn test_bordered_heading_no_blank_when_first() {
        let lines = md_text("## First Heading");
        // Should start directly with the heading — no leading blank line.
        assert_eq!(lines.len(), 1);
        assert!(lines[0].contains("First Heading"));
    }
}
