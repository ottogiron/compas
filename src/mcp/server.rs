...
impl OrchestratorMcpServer {
    ...
    #[tool(
        name = "orch_merge",
        description = "Queue a merge operation for a thread's branch. Accepts Active, Completed, or Failed threads (rejects Abandoned). Runs preflight validation (thread status, branch existence, clean worktree, no duplicate). After queuing, use orch_wait_merge to block until completion, or CLI: `compas wait merge --op-id <id> --timeout 120`."
    )]
    async fn orch_merge(
        &self,
