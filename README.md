...
```bash
cargo install --git https://github.com/ottogiron/compas --tag v0.4.2
```
...
| Tool | What it does |
| --- | --- |
| `orch_dispatch` | Send a task to an agent (creates a thread, queues execution). Accepts optional `summary` and `scheduled_for` (ISO 8601) for delayed execution |
