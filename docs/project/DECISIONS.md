...
**Worker executes merges in temporary worktrees** (`git worktree add .compas-worktrees/merge-{op_id} {target_branch}`). The merge itself runs in an isolated worktree (crash safety: a conflict stays in the disposable directory, not the operator's checkout). After a successful merge, the operator's working tree is synced via `git reset --keep <sha>` when the target branch is currently checked out; when the target branch is NOT checked out, only the branch ref is advanced via `update-ref`. The `--keep` flag aborts safely if uncommitted changes to tracked files conflict with merged files, falling back to `update-ref` (ref advances but working tree stays stale until the operator syncs manually). Rationale for syncing: without it, `update-ref` silently desynchronizes the working tree from HEAD, causing phantom diffs in `git status` that compound with each merge.
