...
Only one worker can run at a time. If a worker is already running, the dashboard detects it and skips spawning a second one. When the dashboard exits, it sends SIGTERM to the embedded worker, which drains in-flight executions and shuts down.
...
- **Recently Completed** -- the 12 most recent completed threads

The status bar shows aggregate counts for running, active, and completed work, plus merge queue depth, total cost (USD), and token usage (in/out). Stale active threads appear dimmed.
...
- Config file path and database path
- Poll interval and agent count
- Log directory
...
Opened by pressing `c` on a thread in the Ops tab. Shows the full dispatch-reply-handoff message history.
